...
cat_cols = ["Month", "DayofMonth", "DayOfWeek", "UniqueCarrier", "Origin", "Dest", "CarrierHour"]
num_cols = ["DepTime", "Distance", "DepHour", "DepMinute"]
...
def prepare(df):
    ...
    df["CarrierHour"] = df["UniqueCarrier"].astype(str) + "-" + df["DepHour"].astype(str)
...
_X_tmp, _ = prepare(train)
cat_levels = {col: sorted(_X_tmp[col].dropna().unique().tolist()) for col in cat_cols}
