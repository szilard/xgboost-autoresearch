...
def prepare(df):
    ...
    dep_time = pd.to_numeric(X["DepTime"], errors="coerce").fillna(0).astype(int)
    dep_hour = (dep_time // 100) % 24
    dep_minute = (dep_time % 100).clip(0, 59)
    dep_minutes = dep_hour * 60 + dep_minute
    X["DepHour"] = dep_hour
    X["DepMinute"] = dep_minute
    X["DepMinutes"] = dep_minutes
    X["DepTimeSin"] = np.sin(2 * np.pi * dep_minutes / 1440)
    X["DepTimeCos"] = np.cos(2 * np.pi * dep_minutes / 1440)
    X["DepHourCat"] = pd.Categorical(dep_hour, categories=range(24))
    for col in cat_cols:
